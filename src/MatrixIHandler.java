...
public class MatrixIHandler implements IHandler {
    ...
    public void handle(InputStream fromClient, OutputStream toClient) throws IOException, ClassNotFoundException {
        ...
        while (doWork) {
            ...
            switch (objectInputStream.readObject().toString()) {
                ...
                case "mission-two": {
                    if (this.matrix != null) {
                        List<ArrayList<Node<Index>>> listOfShortestPath= new ArrayList<>();
                        int[][] primitiveMatrix = (int[][]) objectInputStream.readObject();
                        this.matrix = new Matrix(primitiveMatrix);
                        this.traversableMatrix = new TraversableMatrix(matrix);
                        ...
                        if (traversableMatrix.getValueTravers(traversableMatrix.getOrigin()) != 0 && traversableMatrix.getValueTravers(traversableMatrix.getDestination()) != 0) {
                            listOfShortestPath = threadLocalSearch.getShortestPaths(traversableMatrix);
                        }
                ...
                case "mission-four": {
                    if (this.matrix != null) {
                        int[][] primitiveMatrix = (int[][]) objectInputStream.readObject();
                        List<ArrayList<Node<Index>>> listOfSimplePath= new ArrayList<>();
                        this.matrix = new Matrix(primitiveMatrix);
                        matrix.printMatrix();
                        System.out.println("--------------------------------------------------------------------------------");
                        this.traversableMatrix = new TraversableMatrix(matrix);
                        ...
                        if (traversableMatrix.getValueTravers(traversableMatrix.getOrigin()) != 0 && traversableMatrix.getValueTravers(traversableMatrix.getDestination()) != 0) {
                            listOfSimplePath = threadLocalSearch.getSimplePaths(traversableMatrix);
                        }
    ...
    private void getListOfSccParallel(ReentrantReadWriteLock readWriteLock, ExecutorService threadPool, ThreadLocalDfsVisit<Index> threadLocalSearch, List<Future<HashSet<Index>>> futureList, HashSet<HashSet<Index>> listOfScc, LinkedList<Index> indicesOfOne) {
        for (int i = 0; i < indicesOfOne.size(); i++) {
            ...
            Callable<HashSet<Index>> callable = () -> {
                ...
                traversableMatrix.setStartIndex(indicesOfOne.get(j)); //set start index of the traversable matrix
                HashSet<Index> singleSCC = threadLocalSearch.traverse(traversableMatrix); // apply dfs for this matrix
                //end critical section

                readWriteLock.writeLock().unlock();
                return singleSCC;
            };
            Future<HashSet<Index>> singleSccRecipe = threadPool.submit(callable); //save a recipe of this task
            futureList.add(singleSccRecipe);// and add this recipe to a list of recipes
        }
        for (Future<HashSet<Index>> hashSetFuture : futureList) {
            HashSet<Index> single = null;
            try {
                single = hashSetFuture.get(); //get the SCC from the recipe in the future list
            } catch (InterruptedException | ExecutionException e) {
                e.printStackTrace();
            }
    ...
    private int isSubmarine(int[][] primitiveMatrix, Iterator<Index> it) {
        ...
        while (it.hasNext()) {
            ...
            minCol = Math.min(minCol, i.column);     // find min index, max index -> and check if the rectangle is full in 1
        }
